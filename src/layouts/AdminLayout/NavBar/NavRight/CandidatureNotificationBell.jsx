...
import { FaBell, FaUserPlus, FaExchangeAlt, FaGavel, FaCheckCircle, FaChevronRight } from 'react-icons/fa';
import { motion, AnimatePresence } from 'framer-motion';
import { getCandidatureNotifications } from '../../../../services/dashboard.service';
...
const CATEGORY_CONFIG = {
  new_candidature: {
    ...
    route: '/manager/manage-membership'
  },
  reschedule_request: {
    icon: FaExchangeAlt,
    color: '#d97706',
    ...
    route: '/manager/reschedule-requests'
  },
  pending_decision: {
    icon: FaGavel,
    color: '#7c3aed',
...
const NOTE_COLORS = ['#2563eb', '#d97706', '#7c3aed', '#20c997'];
...
const buildHoverSummary = (data) => {
  ...
  const parts = [];
  if (data.counts.newCandidatures) parts.push(`${data.counts.newCandidatures} à programmer`);
  if (data.counts.rescheduleRequests) parts.push(`${data.counts.rescheduleRequests} reprog.`);
  if (data.counts.pendingDecisions) parts.push(`${data.counts.pendingDecisions} décision${data.counts.pendingDecisions > 1 ? 's' : ''}`);
  return `${data.total} candidature${data.total > 1 ? 's' : ''} à traiter — ${parts.join(' · ')}`;
};

const CandidatureNotificationBell = () => {
  ...
  const fetchNotifications = async () => {
    try {
      const result = await getCandidatureNotifications();
      ...
      console.error('Erreur lors du chargement des notifications de candidatures:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes candidatureBadgePulse {
          0% { box-shadow: 0 0 0 0 rgba(220, 53, 69, 0.5); }
          70% { box-shadow: 0 0 0 9px rgba(220, 53, 69, 0); }
          100% { box-shadow: 0 0 0 0 rgba(220, 53, 69, 0); }
        }
        .candidature-bell-icon.ringing { animation: candidatureBellRing 0.7s ease-in-out; }
        .candidature-bell-badge { animation: candidatureBadgePulse 2s infinite; }
        .candidature-bell-btn {
          position: relative;
          width: 40px;
          height: 40px;
          display: flex;
        ...
        .candidature-bell-btn:hover { background-color: #f1f3f5; }
        .candidature-bell-btn:active { transform: scale(0.92); }
        .candidature-row { cursor: pointer; transition: background-color 0.15s ease; }
        .candidature-row:hover { background-color: #f8f9fa; }
        .candidature-subrow { cursor: pointer; transition: background-color 0.15s ease; }
        .candidature-subrow:hover { background-color: #f1f3f5; }
      `}</style>
      ...
      >
        <button type="button" className="candidature-bell-btn" onClick={handleBellClick} aria-label="Notifications">
          <FaBell
            size={17}
            className={`candidature-bell-icon ${justIncreased ? 'ringing' : ''}`}
          ...
          {showBadge && (
            <Badge
              bg="danger"
              pill
              className="candidature-bell-badge"
              style={{
                position: 'absolute',
                top: '4px',
                right: '4px',
        ...
        {/* ✅ Panneau au clic — même habillage que le dropdown "Envoyer Message" :
            carte blanche, coins bien arrondis, ombre douce, pas d'en-tête coloré lourd. */}
        <AnimatePresence>
          {open && (
            <>
              {/* Petit triangle arrondi qui pointe vers la cloche, comme une bulle de dialogue */}
              <motion.div
              ...
              >
                <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#1a1a2e' }}>Candidatures</span>
                {hasNotifications && (
                  <span style={{ fontSize: '0.7rem', color: '#8a94a6', fontWeight: 600 }}>{data.total} en attente</span>
                )}
              </div>
              ...
              {loading ? (
                ...
                <div style={{ padding: '26px 14px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '0.82rem', color: '#6c757d' }}>Aucune candidature en attente.</div>
                </div>
              ) : (
                groups.map((group) => {
                  const config = CATEGORY_CONFIG[group.type] || CATEGORY_CONFIG.new_candidature;
              ...
              {hasNotifications && (
                <div
                  className="candidature-row"
                  onClick={() => goTo('/manager/manage-membership')}
                  style={{
                    textAlign: 'center',
                    padding: '10px 8px',
                    marginTop: '4px',
...
export default CandidatureNotificationBell;
